...
def main():
    ...
    model = BaselineModel(plm="bert-base-cased", group_slice=group_slice)
    model.to(device)

    checkpoint = torch.load('../finetune/<path>.pth.tar', map_location=device)
    ...
    # pred
    run_eval_ungrouped(logger, torch.cat(pred_prob, dim=0), group_slice, torch.cat(y_true, dim=0))
